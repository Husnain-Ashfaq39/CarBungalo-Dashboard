...
const InvoiceDetails = () => {
  ...
  // Fetch Products with Pagination to retrieve all products
  const fetchProducts = useCallback(async () => {
    ...
    try {
      do {
        ...
        fetchedProducts = productsResponse.documents;
        allProducts = [...allProducts, ...fetchedProducts];
        offset += limit;
      } while (fetchedProducts.length === limit); // Continue fetching if the limit was reached
  ...
  // Calculate totals with shipping rate
  const subTotal = orderItems.reduce(
    (total, item) => total + item.price * item.quantity,
    0
  );
  const shippingCharge = order.shippingRate || 0;
                    ...
                    <CardBody className="p-4 border-top border-top-dashed">
                      <Row className="g-3">
                        <Col sm={6}>
                          ...
                          <p className="fw-medium mb-2" id="delivery-name">
                            {customerName}
                          </p>
                          <p className="text-muted mb-1" id="delivery-address">
                            {deliveryAddress}
                          </p>
                          <p className="text-muted mb-1">
                            <span>Phone: </span>
                            <span id="delivery-phone">
                              {order.phoneNumber || "N/A"}
                            </span>
                          </p>
                          <p className="text-muted mb-0">
                            <span>Email: </span>
                            <span id="delivery-email">
                              {order.email || "N/A"}
                            </span>
                          </p>
                          {order.deliveryInstructions && (
                            <p className="text-muted mt-2">
                              <span className="fw-semibold">Instructions: </span>
                              {order.deliveryInstructions}
                            </p>
                          )}
                        </Col>
                              ...
                              <tr key={item.$id}>
                                ...
                                <td className="text-start">
                                  <span className="fw-medium">
                                    {productsMap[item.productId] || item.productName || "N/A"}
                                  </span>
                                </td>
                        ...
                        >
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td className="text-end">£{shippingCharge.toFixed(2)}</td>
                            </tr>
